...
def parse_sql_file(filename):
    ...
    for idx, match in enumerate(matches):
        ...
        rows.append(values)
    for row in rows:
        row[COLUMNS.index("lstodr")] = int(row[COLUMNS.index("lstodr")]) if row[COLUMNS.index("lstodr")] else None
    return rows
def write_error_log(row, index, error_msg):
    with open("insert_ALLCODE_error.log", "a", encoding="utf-8") as f:
...
def bulk_insert(rows, batch_size=100):
    ...
    print("⚡ Bulk insert finished. Check insert_ALLCODE_error.log for any failed rows.")
if __name__ == "__main__":
    if os.path.exists("insert_ALLCODE_error.log"):
        os.remove("insert_ALLCODE_error.log")
    rows = parse_sql_file("allcd.sql")
    bulk_insert(rows)