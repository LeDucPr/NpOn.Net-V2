...
public static class TableMySqlExtensions
{
        ...
        this INpOnTableWrapper table, string tableName)
    {
        ...
        foreach (var rowWrapper in rowWrappers.Values)
        {
            ...
            foreach (var colName in columnNames)
            {
                ...
                var (paramValue, npgType) = MySqlUtils.NormalizeForMySqlConnector(cell?.ValueAsObject);

                var p = new MySqlParameter(paramName, paramValue ?? DBNull.Value)
                {
                    MySqlDbType = npgType
                };
                parameters.Add(p);
        ...
        this INpOnTableWrapper table, string tableName)
    {
        ...
        foreach (var rowWrapper in rowWrappers.Values)
        {
            ...
            foreach (var pkColName in pkColumnNames)
            {
                ...
                var (paramValue, npgType) = MySqlUtils.NormalizeForMySqlConnector(cell.ValueAsObject);
                var p = new MySqlParameter(pkParamName, paramValue)
                {
                    MySqlDbType = npgType
                };
                parameters.Add(p);
