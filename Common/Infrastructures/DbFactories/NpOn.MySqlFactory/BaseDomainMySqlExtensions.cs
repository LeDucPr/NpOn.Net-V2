...
{
    public static (string CommandText, List<MySqlParameter> Parameters) ToMySqlParamsInsert
        (this List<BaseDomain> domains, bool isUseDefaultWhenNull = false)
    {
        ...
        for (int i = 0; i < domains.Count; i++)
        {
            ...
            foreach (var (columnName, member, memberType) in mappedMembers)
            {
                ...
                var (paramValue, npgType) = NormalizeForMySqlConnector(raw);
                string param = $"@p_{i}_{cols.Count}";
                cols.Add(columnName);
                paramNames.Add(param);

                var p = new MySqlParameter(param, paramValue ?? DBNull.Value);
                if (npgType.HasValue) p.MySqlDbType = npgType.Value;
                parameters.Add(p);
            }
    ...
    public static (string CommandText, List<MySqlParameter> Parameters) ToMySqlParamsUpdate(
        this List<BaseDomain> domains, bool isUseDefaultWhenNull = false)
    {
        if (domains == null || domains.Count == 0)
            throw new Exception("Empty domain list");
        ...
        for (int i = 0; i < domains.Count; i++)
        {
            ...
            foreach (var (colName, member, memberType) in mappedMembers)
            {
                ...
                var (val, npgType) = NormalizeForMySqlConnector(raw);
                string param = $"@v_{i}_{colName}";
                var p = new MySqlParameter(param, val ?? DBNull.Value);
                if (npgType.HasValue) p.MySqlDbType = npgType.Value;
                parameters.Add(p);
            ...
            for (int j = 0; j < pkMembers.Count; j++)
            {
                ...
                var (pkValue, pkType) = NormalizeForMySqlConnector(pkVal);
                var pkParamObj = new MySqlParameter(pkParam, pkValue);
                if (pkType.HasValue) pkParamObj.MySqlDbType = pkType.Value;
    ...
    public static (string CommandText, List<MySqlParameter> Parameters) ToMySqlParamsMerge(
        this List<BaseDomain> domains, bool isUseDefaultWhenNull = false)
    {
        if (domains == null || domains.Count == 0)
            throw new Exception("Empty domain list");
    ...
    public static (string CommandText, List<MySqlParameter> Parameters) ToMySqlParamsDelete(
        this List<BaseDomain> domains, bool isUseDefaultWhenNull = false)
    {
        if (domains == null || domains.Count == 0)
            throw new Exception("Empty domain list");
        ...
        for (int i = 0; i < domains.Count; i++)
        {
            ...
            for (int j = 0; j < pkMembers.Count; j++)
            {
                ...
                var (val, npgType) = NormalizeForMySqlConnector(raw);
                var p = new MySqlParameter(pkParam, val);
                if (npgType.HasValue) p.MySqlDbType = npgType.Value;
                parameters.Add(p);
                pkConditions.Add($"{pkMember.ColumnName} = {pkParam}");
            }
    ...
    // Convert enums and common types to safe MySqlConnector representations by using the centralized utility
    private static (object? Value, MySqlDbType? DbType) NormalizeForMySqlConnector(object? raw)
    {
        return MySqlUtils.NormalizeForMySqlConnector(raw);
    }
